...
const ProductDetail = () => {
  ...
  // variant options (e.g. sizes from a different colour group) from appearing.
  const availableAttributes = useMemo(() => {
    if (!product?.variants) return {};

    // Collect all attribute keys that exist across any variant
    ...
    allKeys.forEach((targetKey) => {
      // Find variants that match all currently selected attributes EXCEPT targetKey
      const compatibleVariants = product.variants.filter((v) => {
        if (!v.attributes) return false;
        return [...allKeys].every((k) => {
          if (k === targetKey) return true; // skip the key we're computing
          // If the user has selected a value for k, the variant must match it
          const selectedVal = selectedAttributes[k];
          if (!selectedVal) return true; // nothing selected for this key yet
      ...
      compatibleVariants.forEach((v) => {
        if (v.attributes?.[targetKey] !== undefined) {
          values.add(v.attributes[targetKey]);
        }
      });
      attrs[targetKey] = Array.from(values);
    });
